<script lang="ts">
    ...
    // מספר הקבוצות נגזר מאורך המערך: 16 תבניות = 4 קבוצות של 4
    const GROUP_COUNT = Math.ceil(ads.length / 4);
    // עצירה אחרי 3 סבבים מלאים של כל הקבוצות (הקבוצה הפותחת + שאר ההחלפות)
    ...
    // המערך בקבוצות של 4: כל הקבוצות מרונדרות זו על גבי זו ורק הפעילה
    // נראית, כך שההחלפה היא מיזוג עדין בין שתי שכבות ולא החלפת תוכן מול העין.
    let groups = $derived(
