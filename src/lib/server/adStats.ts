...
//
// אחסון: כמו visitStats — קטגוריה פנימית (__ref_ad_stats) באוסף ה-items
...
const STATS_CATEGORY = '__ref_ad_stats';
...
async function loadStats(): Promise<{ id: string | null; store: RawStore }> {
    try {
        ...
        if (!(e instanceof StrapiContentTypeError)) {
            console.error('[referendum] adStats load failed:', e);
        }
    ...
    try {
        ...
        if (id) {
            ...
            const res = await strapiPost<{ data: StrapiItem }>('/api/items', {
                data: {
                    label: 'referendum-ad-stats',
                    category: STATS_CATEGORY,
                    description: '[SYSTEM] מדדי פרסומות — משאלי העם',
                    icon: '📊',
        ...
        console.error('[referendum] adStats flush failed:', e);
    } finally {
        flushing = false;
    }
}
