<script lang="ts">
	...
	// ב-Auth.js signIn('google') הוא גם הרשמה וגם התחברות — אין צורך בנתיב שרת נפרד.
	// welcome=new מפעיל את מסך "ברוכים המצטרפים", בדיוק כמו בהרשמה עם אימייל.
	function oauth(provider: 'google' | 'facebook') {
		...
		signIn(provider, { callbackUrl: `${cbUrl.pathname}${cbUrl.search}${cbUrl.hash}` });
	}
</script>

<svelte:head><title>הרשמה | משאלי העם | יוצאים לחירות</title></svelte:head>
